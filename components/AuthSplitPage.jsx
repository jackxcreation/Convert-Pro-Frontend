...
export default function AuthSplitPage({ supabaseClient } = {}) {
  ...
  // ---------------------
  // Replace these wrappers with your actual auth calls (Supabase examples commented below)
  // ---------------------
  async function signInWithProvider(provider) {
    ...
    try {
      if (supabaseClient) {
        // example: await supabaseClient.auth.signInWithOAuth({ provider })
        await supabaseClient.auth.signInWithOAuth({ provider });
      } else {
        // placeholder: open provider url or call your auth method
        console.log("Call OAuth provider:", provider);
      }
    } catch (err) {
      console.error(err);
  ...
  async function signUpWithEmail() {
    ...
    try {
      ...
      if (supabaseClient) {
        // example: sign up (creates user and sends confirmation email / OTP depending on your Supabase settings)
        ...
        if (res.error) throw res.error;
        // If your flow sends OTP via email or magic link, show OTP step or inform user
        setStep("otp");
        setInfo("Check your email for the verification code or link.");
      } else {
        console.log("signUpWithEmail ->", { name, email, password });
        setStep("otp");
        setInfo("(demo) OTP sent to email@example.com");
      }
    } catch (err) {
      console.error(err);
  ...
  async function verifyOtpAndFinish() {
    ...
    try {
      ...
      if (supabaseClient) {
        // Supabase does not expose a universal 'verify OTP' endpoint from client; often email verification is automatic via link.
        // If you implemented server-side OTP, call your API endpoint here to verify.
        // Example placeholder:
        console.log("verify OTP against backend ->", otp);
      }
      setStep("done");
  ...
  function toggleMode(next) {
    resetMessages();
    setMode(next);
    setStep("form"); // small UX: clear fields when switching
    setPassword("");
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-zinc-100 via-white to-zinc-50 p-6">
      <div className="w-full max-w-4xl shadow-2xl rounded-3xl overflow-hidden relative">
        <div className="grid grid-cols-2 min-h-[520px]">
          {/* LEFT: either Sign In or Signup form depending on mode and animation */}
          <div className="relative bg-white p-10 flex items-center justify-center">
            {/* Content container -> show signin when mode==='signin' or step after animation */}
            {mode === "signin" ? (
              <motion.div
              ...
              >
                ...
                <p className="text-sm text-zinc-500 mb-6">
                  Use your account or sign in with a provider
                </p>
                ...
                <div className="grid gap-3">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="text-xs text-zinc-500 text-center mt-2">
                      Forgot password?{" "}
                      <a className="text-indigo-600">Reset</a>
                    </div>
                  </div>
                </div>
              </motion.div>
              ...
              >
                ...
                <p className="text-sm text-zinc-500 mb-6">
                  Enter your details to create a new account — quick and secure.
                </p>
                <button
            ...
            {mode === "signup" ? (
              ...
              >
                ...
                <p className="text-sm text-white/90 mb-6">
                  Sign up quickly with email or continue with a provider
                </p>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid gap-3">
                      ...
                      <div className="flex items-center gap-3">
                        <button
                          onClick={() => signInWithProvider("google")}
                          className="flex-1 rounded-xl border py-3 bg-white/10"
                        >
                          {" "}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-3">
                      <button
                        ...
                        className="flex-1 rounded-full bg-white text-rose-600 py-3"
                      >
                        Verify
                      </button>
                      <button
                ...
                {step === "done" && (
                  <div className="text-center">
                    ...
                    <button
                      onClick={() => toggleMode("signin")}
                      className="mt-4 rounded-full bg-white text-rose-600 px-6 py-2"
                    >
                      Sign in
                    </button>
                  </div>
              ...
              >
                ...
                <button
                  ...
                  className="rounded-full border border-white/30 px-6 py-3 bg-white/10"
                >
                  Sign up
                </button>
              </motion.div>
        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-white/90 px-4 py-2 rounded-xl shadow-md flex gap-4 items-center text-sm">
          <div className="min-w-[220px] text-xs text-zinc-700">
            {info || "Convert Pro — fast & private converters"}
          </div>
